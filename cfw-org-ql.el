...
(defun jrf/calfw-parser ()
  (-let* (((&alist "CATEGORY" category
    ...
    (setq timestamp-range (alist-get 'active-range (jrf/cfw-org-ql--timestamp-parser 'all)))
    (let (start-date
	  start-time
	  end-date
	  end-time
		   ...
		   collect (cond
			    ...
			    ((and (eq type 'timestamp) timestamp)
			     (cl-loop for tt in timestamp
				      collect (progn (setq start-date (jrf/calfw--convert-date-string tt))
						     (setq start-time (jrf/calfw--convert-date-string tt 'time))
						     (when (string-match (rx "-" (group (= 2 digit) ":" (= 2 digit)))
									 tt)			   
						       (setq end-date start-date)
						       (setq end-time (cl-loop for each in (s-split ":" (match-string 1 tt) t)
			    ...
			    ((and (eq type 'timestamp-range) timestamp-range)
			     (cl-loop for tt in timestamp-range
				      collect (progn (setq start-date (jrf/calfw--convert-date-string (car tt)))
						    ...
						    (setq end-date (jrf/calfw--convert-date-string (cdr tt)))
						    (setq end-time (jrf/calfw--convert-date-string (cdr tt) 'time))
						    (make-cfw:event :title (progn
									     (add-face-text-property 0 (length todo) 'org-todo nil todo)
									     (propertize 
									      (concat
